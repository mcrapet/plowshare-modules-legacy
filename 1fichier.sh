...
1fichier_download() {
    ...
    local FID PAGE FILE_URL FILE_NAME WAIT CV SESS ADZ DL_PAGE
    ...
    # The download page needs the "adz" post parameter to show
    if [ -z "$FILE_URL" ]; then
        if [[ $PAGE =~ name=\"adz\"[[:space:]]value=\"([^\"]*)\" ]]; then
            ADZ=${BASH_REMATCH[1]}
            PAGE=$(curl -F "adz=$ADZ" -b "$COOKIE_FILE" -b 'LG=en' "$URL")
        fi
