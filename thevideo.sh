...
MODULE_THEVIDEO_REGEXP_URL='https\?://\(www\.\)\?thevideo\.me/'

MODULE_THEVIDEO_UPLOAD_OPTIONS="
...
MODULE_THEVIDEO_UPLOAD_REMOTE_SUPPORT=no
...
thevideo_upload() {
    ...
    local -r FILE=$2
    local -r DESTFILE=$3
    local -r BASE_URL='https://thevideo.me'
    ...
    if CV=$(storage_get 'cookie_file'); then
        ...
        thevideo_login "$AUTH" "$COOKIE_FILE" "$BASE_URL" || return
    ...
    PAGE=$(curl_with_log \
        ...
        "${FORM_ACTION}${UPLOAD_ID}&utype=reg&disk_id=${FORM_DSK_ID}" | break_html_lines) || return
